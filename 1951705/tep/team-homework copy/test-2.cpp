...
int main() // int argc, char *argv[])
{
...

    {
        ...
        body += "提交\r\n";
        body += "--" + boundary + "\r\n";
        body += "Content-Disposition: form-data; name=\"upfile6\"; filename=\"test6-socket-tcp-sync.pdf\"\r\n";
        body += "\r\n";
        string file = FileToStr("./test-submit/test6-socket-tcp-sync.pdf");
        if (file == "")
            return -1; //小心文件里面有\0
        body += file;
        body += "\r\n";
        ...
        cout << "file len:" << file.length() << endl;

        fstream f1("file.txt", ios::out | ios::binary);
        if (f1.good())
        {
